...
Rpc._parseTxResult = function(info) {
  ...
  info.vout.forEach( function(o) {
    ...
    if (info.isCoinBase && o.hasOwnProperty('scriptPubKey') && o.scriptPubKey.hasOwnProperty('addresses')) {
      cbAddresses += o.scriptPubKey.addresses.join();
    }
    delete o.scriptPubKey.hex;
  });
  info.valueOut = valueOutSat.toFixed(0) / bitcore.util.COIN;
  info.size     = b.length;
  info.cbAddresses = cbAddresses;
