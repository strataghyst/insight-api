...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and GuldenD? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.getBitcoinCoreVersion(function(bitcoinVersion) {
    if (bitcoinVersion > 100000 && !config.forceRPCsync) {
      info('-------------------------------------------------------');
      info('- Gulden Core version >0.10 only works with RPC sync -');
      info('- Set the env variable INSIGHT_FORCE_RPC_SYNC = 1     -');
      info('-------------------------------------------------------');
      process.exit(1);
    } else {
      info('Gulden Core version ', bitcoinVersion);
      info('Using RPC sync ');
    }
  });
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if (self.blockChainHeight === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on GuldenD. Please reset Insight DB. Tip was:' + tip)
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at GuldenD');

  if (self.blockExtractor) {
    info('GuldenD dataDir configured...importing blocks from .dat files');
